h00134
s 00560/00000/00000
d D 1.1 19/04/02 14:02:47 wrc 1 0
c date and time created 19/04/02 14:02:47 by wrc
e
u
U
f e 0
...
int32_t spawn( int (*entry)(int,void*,void*), uint8_t prio,
    ...
    // we are the child
    uint32_t status = exec( entry, prio, argc, arg1, arg2 );
    ...
    pid = getpid();
    sprint( buf, "PID %d exec() failed, status %d\n", pid, status );
    cwrites( buf );
...
int cwrite( const char *buf, uint32_t size ) {
   return( write(STR_CONS,buf,size) );
}
